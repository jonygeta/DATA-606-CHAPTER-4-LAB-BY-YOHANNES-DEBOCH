...
We consider real estate data from the city of Mes, Iowa.  The details of 
every real estate transaction in Mes is recorded by the City Assessor's 
office.  Our particular focus for this lab will be all residential home sales 
in Mes between 2006 and 2010.  This collection represents our population of 
...
Answer: Based on the histogram we can say that the population is slightly skewed to the right.
...
If we were interested in estimating the mean living area in Mes based on a 
...
hist(samp1)
```
This distribution is bi modal and it's average is greater than the previous sample.
If we're interested in estimating the average living area in homes in Mes 
...
summary(samp2)
```
Histogram of second sample
```{r}
hist(samp2)
```
The second sample is uni-modal and right skewed while the first sample is bi modal . The second sample has a mean 1535  which is greater than the first sample mean 1491.
I think the sample of size 1000 will give more accurate result.
...
```
5000 elements are in sample_means50.
The distribution is approximately normally distributed. 
The mean is 1500. 
With a sample of 50,000 the distribution will be more closer to normality but it won't differ much from the previous sample of 5000.
...
the average living area in homes in Mes.  Because the sample mean is an 
...
Answer: When the sample size is larger the mean tends towards the population mean. The spread becomes similar to approximately normal shape. 
* * *
## On your own

So far, we have only focused on estimating the mean living area in homes in 
Mes.  Now you'll try to estimate the mean home price.
...
mean(price_sample)
```
My point estimate of price is 183136.6.
...
for(i in 1:5000){
   ...
   sample_means50[i] <- mean(sample_price)
}
mean(sample_means50)
hist(sample_means50)
```
The mean home price would be 180000. The population mean is 180807.4.
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    be the mean sale price of homes in Mes?
Answer:
```{r}
sample_means150 <- rep(0, 5000)

for(i in 1:5000){
   sample_price <- sample(price, 150)
   sample_means150[i] <- mean(sample_price)
}
mean(sample_means150)
hist(sample_means150)
```
The mean home price would be 185000. The population mean is 180719.
...
Answer: Sampling distribution 3 has smaller spread out of sampling distribution 2,3.
